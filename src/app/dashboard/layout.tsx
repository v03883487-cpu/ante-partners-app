    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      <nav className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 px-6 py-4 sm:px-12">
        <div className="flex items-center gap-8">
          <Link href="/dashboard" className="font-display text-sm font-black tracking-tight text-white">
            ANTE <span className="text-zinc-400">PARTNERS</span>
          </Link>
          <div className="hidden items-center gap-6 text-sm text-zinc-400 sm:flex">
            ...
            {partner?.is_admin && (
              <Link href="/admin" className="font-semibold text-white transition-opacity hover:opacity-60">
                Админка
              </Link>
            )}
          </div>
