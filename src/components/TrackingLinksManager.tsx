    ...
    <div>
      <form onSubmit={handleCreate} className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={creating}
          className="shrink-0 rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-[#0a0a0a] disabled:opacity-50"
        >
          Создать
        </button>
      </form>
      ...
      <div className="mt-6 divide-y divide-white/10 border-y border-white/10">
        {links.length === 0 && <p className="py-6 text-sm text-zinc-400">Ссылок пока нет — создайте первую выше.</p>}
        {links.map((l) => (
          <div key={l.id} className="flex flex-wrap items-center justify-between gap-3 py-4">
            <div>
              ...
              <code className="text-xs text-zinc-400">{AGENCY_SITE_URL}/?ref={l.code}</code>
            </div>
